...
print(f"3. Launching direct absolute simulation for {FORECAST_MONTHS} months...")
# Prendre la dernière ligne connue de chaque transect comme base statique
last_known_state = df.drop_duplicates(subset=['segment_id'], keep='last').copy()
current_date = last_known_state['date'].max()

# On stockera la toute dernière position calculée pour l'export SIG
final_positions = {}
...
for step in range(1, FORECAST_MONTHS + 1):
    ...
    step_df['year'] = current_year
    step_df['month'] = current_month
    
    # Inject standard weather climatology
    ...
    # Batch Prediction of ABSOLUTE position
    if mask_coast.sum() > 0:
        X_c = step_df.loc[mask_coast, training_features]
        step_df.loc[mask_coast, 'pred_position_m'] = model_coast.predict(X_c)
        
    if mask_river.sum() > 0:
        X_r = step_df.loc[mask_river, training_features]
        step_df.loc[mask_river, 'pred_position_m'] = model_river.predict(X_r)
        
    # State update for graphs and GIS
    for _, row in step_df.iterrows():
        seg = row['segment_id']
        pos = row['pred_position_m']
        final_positions[seg] = pos
        
        if seg in TRANSECTS_TO_PLOT:
            forecast_trajectories[seg]['dates'].append(current_date)
            forecast_trajectories[seg]['positions'].append(pos)
...
gis_df['final_position_m'] = gis_df['segment_id'].map(final_positions)
gis_df['total_predicted_change_m'] = gis_df['final_position_m'] - gis_df['cross_distance_m']
...
cols_to_keep = [
    ...
    'cross_distance_m', 'final_position_m', 'total_predicted_change_m', 'geometry'
]
...
for seg in TRANSECTS_TO_PLOT:
    ...
    plot_history = history_data.tail(48)
    plt.plot(plot_history['date'], plot_history['cross_distance_m'], label='Known History', color='black', marker='o', markersize=4)
    
    pred_dates = forecast_trajectories[seg]['dates']
    pred_vals = forecast_trajectories[seg]['positions']
    
    # Connexion visuelle entre l'historique et la prédiction
    connect_dates = [plot_history['date'].iloc[-1]] + pred_dates
    connect_vals = [plot_history['cross_distance_m'].iloc[-1]] + pred_vals
    
    plt.plot(connect_dates, connect_vals, label=f'AI Direct Projection ({FORECAST_MONTHS} months)', color='red', linestyle='--', marker='x', markersize=6)
    
    zone_label = "River/Estuary" if history_data['in_river_zone'].iloc[0] == 1 else "Coast (Beach)"
    plt.title(f"Prospective Absolute Forecast - Transect: {seg} ({zone_label})")
    plt.xlabel("Date")
    plt.ylabel("Absolute Cross Distance (m)")
